...
export function VotingView({ user }: { user: any }) {
  ...
  useEffect(() => {
    if (game?.status !== "voting" || !game.votingStartTime) return;
  ...
  }, [game?.status, game?.votingStartTime, game?.ownerId, user.uid]);
      ...
      {game.status === "meeting" ? (
         <div className="flex flex-col items-center justify-center relative z-10 w-full max-w-md">
           <div className="flex items-center justify-center gap-2 mb-8 mt-4">
             <span className="text-red-400 font-bold tracking-widest text-lg uppercase border border-red-500/30 bg-red-500/10 py-2 px-4 rounded-full text-center">
               RENDEZ-VOUS AU CHECK POINT !
             </span>
           </div>
           <p className="text-zinc-400 mb-8 max-w-sm text-center">Wait for all operatives to gather before voting begins.</p>
           
           {game.ownerId === user.uid && (
             <button onClick={startVoting} className="mt-4 px-8 py-4 bg-red-500 hover:bg-red-600 font-bold rounded-full shadow-lg w-full max-w-md relative z-10 cursor-pointer uppercase tracking-widest transition-colors mb-8">
               ALL ACCOUNTED FOR - START VOTE
             </button>
           )}
         </div>
      ) : (
        <>
          <p className="text-red-500 font-bold text-2xl mb-8 mt-4 relative z-10 font-mono">00:{timeLeft.toString().padStart(2, '0')}</p>
